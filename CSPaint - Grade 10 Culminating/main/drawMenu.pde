...
void drawMenu() {
  ...
  loadWork(); //creates rectangle for load
...
void loadWork() {
  if (oldSketch != null && loaded == true) {
    image(oldSketch, (width+menuX)/2, height/2);
    loaded = false;
  }
  fill(255);
  rect(menuX*0.5, menuY*0.85, menuX*0.5, menuY*0.1);

  fill(0);
  text("Load", menuX*0.75, menuY*0.905);
}
void clearScreen() {
  textMode(CENTER);
  fill(255);
